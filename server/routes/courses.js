...
router.get('/', function(req, res, next) {
  mysql.query('SELECT name, level, duration, date, place, price, photo_course, course_id, teacher FROM courses LEFT JOIN courses_x_teachers ON courses.id = courses_x_teachers.course_id LEFT JOIN teachers ON courses_x_teachers.teacher_id = teachers.id', function(error, result, fields) {
    res.json(result);
  });
});
...
router.post('/filter', function(req, res, next) {
  ...
  if(req.body.teacherFilter !== "All" && req.body.teacherFilter.length) {
    filterObject.push(" teacher = '" + req.body.teacherFilter + "' ");
  }
