...
const OurCoursesFilters = () => {
    ...
    const createFilterList = filter => {
        const options = removeDuplicates(courses.map(item => item[filter]));
        const optionsTable = ["All", ...options];
        return createOption(optionsTable);
    }

    const filterTeachersList = createFilterList("teacher");
    const filterLevelsList = createFilterList("level");
    const filterLocationsList = createFilterList("place");
